...
const PATTERNS = {
  ...
  personalInfo: {
    ...
    // 한국 특화 패턴
    koreanSSN: /\b\d{6}[-\s]?[1-4]\d{6}\b/g, // 주민등록번호 (앞 6자리-뒷 7자리, 1-4로 시작)
    koreanBizNo: /\b\d{3}[-\s]?\d{2}[-\s]?\d{5}\b/g, // 사업자등록번호 (000-00-00000)
    koreanForeignerNo: /\b\d{6}[-\s]?[5-8]\d{6}\b/g, // 외국인등록번호 (5-8로 시작)
    koreanDriverLicense: /\b\d{2}[-\s]?\d{6}[-\s]?\d{2}\b/g, // 운전면허번호
    koreanBankAccount: /\b\d{3,4}[-\s]?\d{2,4}[-\s]?\d{4,6}[-\s]?\d{0,3}\b/g, // 계좌번호 (다양한 형식)
  },
